...
이 파이프라인은 **Ralph TUI**가 오케스트레이션하고, **Claude Code / OpenAI Codex CLI** 중 원하는 에이전트가 연구 에이전트로 동작합니다. 두 에이전트 모두 Anthropic 의 공개 **Agent Skills 표준**을 따르므로 동일한 `/research-*` 슬래시 명령이 그대로 동작합니다. 연구자가 다룰 주제만 정해주면, 관련 논문을 빠짐없이 검색·수집하고, 전문을 확보해 방법론·결과·한계를 분석한 뒤, 최종적으로 **노션(Notion) 데이터베이스**로 정리된 리포트까지 산출합니다.
...
1. **에이전트가 확인 없이 로컬 명령을 실행합니다.**
   `scripts/run-agent.sh`는 Claude Code를 `--dangerously-skip-permissions`, Codex를 **`--dangerously-bypass-approvals-and-sandbox`** 로 기동합니다. Ralph TUI가 각 태스크마다 새 세션을 띄우며, 에이전트는 `Bash` / `Write` / `Edit` / `WebFetch` 등을 허용 프롬프트 없이 실행합니다. **공용 PC, 프로덕션 계정, 민감 레포가 있는 홈 디렉토리**에서는 실행하지 마세요. 별도 계정 또는 전용 작업 디렉토리를 권장합니다.

   - Claude 안전 모드: `CLAUDE_SAFE_MODE=1 ./start-research.sh ...` — `.claude/settings.json` 의 `permissions.allow`/`deny` 가 적용되어 파이프라인 내부 스크립트만 허용하고 `sudo`/`curl`/`rm -rf /` 등을 차단합니다.
   - Codex 안전 모드: `CODEX_SAFE_MODE=1 ./start-research.sh ...` — Codex 기본 sandbox(`workspace-write`) 로 돌립니다. 외부 DNS · 헤드리스 브라우저가 막히므로 Tier 1/2 전문 수집은 실패하지만(Tier 3 MCP 로 우회) 파일 쓰기 범위가 좁아집니다. 자세한 내용은 아래 **Codex 실행 시 보안 모델** 섹션 참조.
...
- **Node.js** (scripts 실행용)
- **연구 에이전트 CLI** — 아래 둘 중 최소 하나 (실행 시 전환 가능)
...
```bash
ls -1t .ralph-tui/iterations/ | head  # Ralph per-iteration 로그(최신순, TUI 모드)
...
| `❌ 필수 CLI 누락: 'claude'` (또는 codex) | 선택한 에이전트 CLI 미설치. `--agent` 로 다른 에이전트로 바꾸거나 설치 |
...
## 에이전트 선택 (Claude / Codex)

이 파이프라인은 Anthropic의 **Agent Skills 공개 표준**을 채택해 두 가지 에이전트
CLI에서 동일한 슬래시 명령(`/research-*`)을 수행할 수 있습니다.

### 정본과 파생물

스킬 정본은 `.claude/skills/` 하나뿐이며, `npm install` 또는
`npm run sync-agents` 실행 시 `scripts/sync-agent-assets.mjs` 가 Codex 파생물을
자동 생성합니다.
...
| `.env` 의 `AGENT` 변수 | `AGENT=codex` | 2순위 |
| 지정 없음 | — | 기본값 `claude` |
...
두 에이전트 모두 각자의 로그인/API 키 절차를 먼저 완료해야 합니다 — 본 파이프라인은
로그인 자체를 대행하지 않습니다.

### 호환성 노트

- Claude Code 는 `CLAUDE.md` 를, Codex 는 `AGENTS.md` 를 자동으로 읽습니다.
  두 파일은 같은 원칙을 담고 있으며 `AGENTS.md` 가 에이전트 공용 본문입니다.
- Ralph TUI 는 `run-agent.sh` 래퍼와 `--agent` 플래그를 통해 두 에이전트 모두
...
### 왜 Codex sandbox 를 푸는가

Codex CLI 기본값(`--full-auto` = `workspace-write` sandbox) 은 LLM 이 생성한 명령의 부작용을 macOS seatbelt 로 가둡니다. 하지만 이 파이프라인이 하는 일이 **본질적으로 sandbox 와 충돌**합니다:

| 티어 | 필요 자원 | Codex 기본 sandbox 에서 벌어지는 일 |
|------|----------|-------------------------------------|
| Tier 1 (API) | OpenAlex / Semantic Scholar / Unpaywall / arXiv 로 외부 HTTPS 호출 | **DNS lookup 자체가 차단** — `ENOTFOUND`, `EAI_AGAIN` 으로 즉시 실패 |
| Tier 2 (Browser) | Playwright 가 Chromium 자식 프로세스 spawn | **Mach port rendezvous 생성 차단** — Chromium 이 `Permission denied (1100)` 으로 즉사 |
| Tier 3 (MCP) | 이미 실행 중인 Playwright MCP 서버에 IPC 로 위임 | ✅ 영향 없음 (별도 프로세스) |

즉 기본 sandbox 로 돌리면 **전문 수집의 80% 이상이 즉시 실패**하고 모든 논문이 Tier 3 로 몰리는데, Tier 3 는 Claude 호출마다 `~50K` 토큰을 씁니다. 실용적으로 감당이 안 됩니다.

### 현재 기본 동작

이 저장소는 Codex 를 다음 두 지점에서 **sandbox 없이** 실행합니다:

- `scripts/run-agent.sh` (Phase 0 인테이크, Phase 1 태스크 생성): `codex --dangerously-bypass-approvals-and-sandbox`
- `.ralph-tui/config.toml` 의 `[agentOptions]` (Phase 2 본조사): `sandbox = "danger-full-access"` · `fullAuto = true`

이 상태에서 Codex 는:
- 임의의 로컬 파일을 읽고 쓸 수 있음 (`~/` 전체, 시스템 파일 포함)
- 임의의 외부 도메인에 네트워크 호출 가능
- 임의의 서브프로세스를 spawn 가능 (`curl`, `git push`, `rm` 등)
- 모든 approval 프롬프트를 건너뜀

프롬프트 인젝션이 들어온 논문을 읽을 때, 에이전트가 그 지시대로 `~/.ssh/id_rsa` 를 외부로 전송하거나 Git 저장소를 조작하는 일도 **물리적으로 막히지 않습니다**.

### 언제 이 기본값으로 돌려도 되는가

다음 조건이 **모두** 참일 때만 기본값을 유지하세요:

- ✅ 본인 소유 개인 머신
- ✅ 본인 단독 사용 (팀원/클라이언트 데이터 혼재 없음)
- ✅ 연구 주제가 내부 공개 자료 기반 (기업 기밀이 딸린 주제 아님)
- ✅ 결과물(`findings/`) 을 본인만 검토
- ✅ 머신에 외부 비밀(API 키, SSH 키, 2차 인증 백업 등) 이 있더라도 **노출되어도 손실이 감당 가능**

하나라도 어긋나면 **안전 모드** 로 돌리세요.

### 안전 모드 전환 방법

```bash
# Codex 만 sandbox 로 돌리기 (Ralph 내부는 별도 — 아래 참고)
CODEX_SAFE_MODE=1 ./start-research.sh deep --agent codex

# 아예 Claude 로 갈아타기 (추천 — 파이프라인 제약 가장 적음)
./start-research.sh deep --agent claude
```

Ralph 내부의 Codex 까지 sandbox 로 되돌리려면 `.ralph-tui/config.toml` 의 `[agentOptions]` 에서 다음 두 줄을 수정하세요:

```toml
sandbox = "workspace-write"   # 기본 sandbox 복원
fullAuto = true               # 유지
```

이 경우 Tier 1/2 전문 수집은 실패하고 Tier 3 (Playwright MCP) 로만 우회되어, **토큰 비용이 훨씬 커지지만 파일·네트워크 접근은 sandbox 로 제한**됩니다.

### 팀/공용 환경 권장 조합

- `--agent claude` + `CLAUDE_SAFE_MODE=1` + `.claude/settings.json` 의 `permissions.deny` 확장
- 이 조합이 본 파이프라인에서 가장 보수적이며, 실제 수집 성공률도 가장 높습니다. Claude Code 는 자체 sandbox 가 없어 DNS/브라우저가 막히지 않습니다.

### 정말 무슨 일이 생겼는지 확인하려면

- 에이전트가 실행한 bash 명령은 `.ralph-tui/iterations/*.log` 와 `logs/codex-exec-*.log` 에 전부 남습니다.
- 완료 후 `grep -E "curl|wget|ssh|scp|rm " logs/codex-exec-*.log` 로 의심 패턴을 검사하세요.
- `SECURITY.md` 에 제보 경로가 있습니다.
...
Codex 경로에서는 Claude 고유의 이벤트 훅이 동작하지 않으므로, 같은 보장을
...
│   ├── run-agent.sh               # claude/codex 호출 래퍼
│   ├── sync-agent-assets.mjs      # .claude/skills → .codex 동기화
│   ├── hooks/                     # 자동화 훅 구현체
│   └── lib/
